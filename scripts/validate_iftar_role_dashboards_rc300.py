...
def main():
    ...
    assert 'if mode in {"management", "site", "supervisor"}:' in backend
    ...
    assert 'data-assign="${esc(o.project)}"' in page
    assert "إسناد فريق المشروع الثابت" in page
    assert "لم يسجل فريق المشرفين" in page
    for field in (
