...
frappe.ui.form.on("WAFD Hotel Undertaking", {
  ...
  refresh(frm) {
    ...
    if (frm.doc.docstatus === 0) frm.add_custom_button(__("حفظ بيانات الشركة كافتراضية"), () => frappe.call({method:"wafd_one.wafd_one.doctype.wafd_hotel_undertaking.wafd_hotel_undertaking.save_company_defaults",args:{name:frm.doc.name},freeze:true,callback(){frappe.show_alert({message:__("تم حفظ بيانات الشركة الافتراضية"),indicator:"green"});}}), __("الإجراءات"));
