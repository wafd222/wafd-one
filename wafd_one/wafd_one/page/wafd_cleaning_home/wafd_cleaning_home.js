frappe.pages["wafd-cleaning-home"].on_page_load = function (wrapper) {
  ...
  function usageCards(){ return !(data.recent_usage||[]).length ? empty() : data.recent_usage.map(u=>`<article><div><b>${esc(localData(u.purpose))}</b><small>${esc(localData(u.location||""))} · ${esc(u.usage_date)}</small></div><span>${(u.items||[]).map(i=>`${esc(localData(i.ingredient))}: ${esc(i.quantity)} ${esc(localData(i.uom))}`).join(lang === "ar" ? "، " : ", ")}</span></article>`).join(""); }
