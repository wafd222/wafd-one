frappe.pages["wafd-driver-trips"].on_page_load = function (wrapper) {
  ...
  function renderTrips() {
    ...
    $root.find("#wafd-driver-list").html(`<div class="wafd-trip-list">${trips.map((trip) => {
      ...
      let actions = "";
      if (["مخططة / Planned", "تم التحميل / Loaded"].includes(trip.status) || (trip.status === "متأخرة / Delayed" && !trip.actual_departure)) actions += `<button type="button" data-action="start" data-trip="${esc(trip.name)}">${esc(tr("start"))}</button>`;
      if (trip.status === "في الطريق / In Transit" || (trip.status === "متأخرة / Delayed" && trip.actual_departure)) actions += `<button type="button" class="secondary" data-action="arrive" data-trip="${esc(trip.name)}">${esc(tr("mark_arrived"))}</button>`;
