frappe.pages["wafd-iftar-driver"].on_page_load = function (wrapper) {
  ...
  const T = {
    ...
    bread:{ar:"الخبز",en:"Bread bags"},
    carts:{ar:"العربيات",en:"Carts"},
    tablecloths:{ar:"السفر",en:"Tablecloths"},
    waste_bags:{ar:"أكياس النفايات",en:"Waste bags"},
    gloves:{ar:"القفازات",en:"Gloves"},
    masks:{ar:"الكمامات",en:"Masks"},
    shoe_covers:{ar:"غطاء الأرجل",en:"Shoe covers"},
  ...
  function renderTrips() {
    ...
    $root.find("#wafd-driver-list").html(`<div class="wafd-trip-list">${visibleTrips.map((trip) => {
      ...
      const supplyItems = [["cartons",trip.iftar_cartons],["bread",trip.iftar_bread_quantity],["carts",trip.iftar_carts],["tablecloths",trip.iftar_tablecloths],["waste_bags",trip.iftar_waste_bags],["gloves",trip.iftar_gloves],["masks",trip.iftar_masks],["shoe_covers",trip.iftar_shoe_covers]].map(([key,value])=>`<div class="wafd-trip-info"><small>${esc(tr(key))}</small><b>${esc(Number(value||0))}</b></div>`).join("");
      return `<article class="wafd-trip-card ${sequenceState === "locked" ? "is-locked" : ""} ${sequenceState === "missed" ? "is-missed" : ""}">${sequenceNote}<div class="wafd-trip-head"><h3>${esc(hotelName(trip))}</h3><span class="wafd-trip-status">${esc(tripStatus(displayStatus))}</span></div><div class="wafd-trip-grid">${isManager ? `<div class="wafd-trip-info"><small>${esc(tr("driver"))}</small><b>${esc(trip.driver || "—")}</b></div>` : ""}${trip.vehicle ? `<div class="wafd-trip-info"><small>${esc(tr("vehicle"))}</small><b>${esc(trip.vehicle)}</b></div>` : ""}${Number(trip.quantity)>0 ? `<div class="wafd-trip-info"><small>${esc(tr("quantity"))}</small><b>${esc(trip.quantity)}</b></div>` : ""}${trip.meal_type ? `<div class="wafd-trip-info"><small>${esc(tr("meal"))}</small><b>${esc(mealName(trip.meal_type))}</b></div>` : ""}<div class="wafd-trip-info"><small>${esc(tr("arrival"))}</small><b>${esc(fmtDate(trip.planned_arrival))}</b></div>${loading.seal_number ? `<div class="wafd-trip-info"><small>${esc(tr("seal"))}</small><b>${esc(loading.seal_number)}</b></div>` : ""}${supplyItems}</div>${loading.loading_photo ? `<div class="wafd-loading-evidence"><img src="${esc(loading.loading_photo)}" alt="${esc(tr("loading_photo"))}"><div><b>${esc(tr("loading_photo"))}</b><small>${esc(tr("uploaded_by"))}: ${esc(loading.loading_photo_uploaded_by || loading.supervisor || "—")}</small></div></div>` : ""}${proof?.delivery_photo ? `<div class="wafd-loading-evidence"><a href="${esc(proof.delivery_photo)}" target="_blank"><img src="${esc(proof.delivery_photo)}" alt="${esc(tr("proof"))}"></a><div><b>${esc(tr("delivered"))}</b><small>${esc(fmtDate(proof.delivery_time))}</small></div></div>` : ""}<div class="wafd-trip-actions">${actions}${trip.map_url ? `<a href="${esc(trip.map_url)}" target="_blank" rel="noopener">${esc(tr("open_map"))}</a>` : ""}</div></article>`;
