...
async function build_report_center(wrapper){
  ...
  $r.find('.irc-search-project').html(`<label class="control-label">المشروع</label><select class="form-control irc-project-search-select"><option value="">— كل المشاريع —</option></select>`);
  const projectValue=()=>($r.find('.irc-project-search-select').val()||'');
  ...
  const secondary=[
    ...
    {icon:'▥',title:'لوحة التشغيل اليومية',desc:'متابعة المشروع من الإنتاج حتى الاستلام',type:'page',page:'wafd-iftar-team'}
  ];
  ...
  async function runSearch(autoSelect=false){
    const response=await frappe.call({method:'wafd_one.wafd_one.iftar_pro.search_iftar_projects',args:{project:projectValue()||null,table_owner:ownerControl.get_value()||null,date:dateControl.get_value()||null,limit:200},freeze:true,freeze_message:'جارٍ البحث عن المشاريع...'});
    ...
    if(lastResults.length){
      ...
      $r.find('.irc-project-search-select').html(`<option value="">— كل المشاريع —</option>${options}`);
      $r.find('.irc-results').html(`<div class="irc-results-head"><b>المشاريع المتاحة</b><span>${lastResults.length} مشروع</span></div><div class="irc-project-picker-wrap"><label>اختر المشروع لفتح التقارير</label><select class="form-control irc-project-picker"><option value="">— اختر مشروعاً —</option>${options}</select></div>`);
      if(current && lastResults.some(x=>x.name===current)){
        $r.find('.irc-project-search-select,.irc-project-picker').val(current);
        selectProject(current);
      } else if(autoSelect && lastResults.length===1){
        $r.find('.irc-project-search-select,.irc-project-picker').val(lastResults[0].name);
        selectProject(lastResults[0].name);
      }
    }else{
  ...
  $section.on('click.wafdReportCenter','.irc-clear-btn',()=>{$r.find('.irc-project-search-select').val('');ownerControl.set_value('');dateControl.set_value('');selectedProject='';selectProject('');runSearch(false);});
  $section.on('click.wafdReportCenter','.irc-result',function(){selectProject($(this).data('project'));});
  $section.on('change.wafdReportCenter','.irc-project-picker',function(){const v=this.value||'';$r.find('.irc-project-search-select').val(v);selectProject(v);});
  $section.on('change.wafdReportCenter','.irc-project-search-select',function(){const v=this.value||'';$r.find('.irc-project-picker').val(v);selectProject(v);});
  ...
  $section.on('click.wafdReportCenter','.irc-back',()=>frappe.set_route('wafd-iftar-team'));

  if(routeOptions.project){await runSearch(true);}
