frappe.pages["wafd-iftar-team"].on_page_load = function(wrapper) {
  ...
  function operations(){
    ...
    return `<div class="ift-grid">${data.operations.map(o=>{
      ...
      if(data.mode==='management') actions+=`<button class="btn btn-success" data-finalize="${esc(o.name)}">اعتماد التقرير الرسمي</button>`;
  ...
  $root.on('click','[data-generate]',async function(){const res=await frappe.call({method:'wafd_one.wafd_one.iftar_team.ensure_supervisor_reports',args:{operation_name:$(this).data('generate')},freeze:true});const x=res.message||{};if((x.skipped_without_user||[]).length)frappe.msgprint(`لم تُنشأ تقارير لمن لم يربط حسابهم بالخطة: ${x.skipped_without_user.join('، ')}`);load();});
