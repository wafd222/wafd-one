...
function build_wizard(wrapper) {
  ...
  const definitions = [
    ...
    {step:4, fieldtype:'Link', fieldname:'project_manager_user', label:'مدير المشروع', options:'User', get_query:()=>({filters:{enabled:1,user_type:'System User'}})},
    {step:4, fieldtype:'Link', fieldname:'kitchen_supervisor_user', label:'مشرف المطبخ', options:'User', get_query:()=>({filters:{enabled:1,user_type:'System User'}})},
    {step:4, fieldtype:'Link', fieldname:'delivery_supervisor_user', label:'مشرف التوصيل', options:'User', get_query:()=>({filters:{enabled:1,user_type:'System User'}})},
    {step:4, fieldtype:'Link', fieldname:'site_manager_user', label:'مدير الموقع', options:'User', get_query:()=>({filters:{enabled:1,user_type:'System User'}})},
