    ...
    if frappe.db.exists("DocType", "WAFD Document Template"):
        ...
        for row in rows:
            ...
            for block in canvas.get("blocks") or []:
                if _looks_like_terms(block):
                    block["html"] = TERMS_HTML
                    block["text"] = TERMS_HTML
                    block["content"] = TERMS_HTML
                    for size_key in ("height", "h"):
                        try:
                            current = int(float(block.get(size_key) or 0))
                        except Exception:
                            current = 0
                        if current and current < 225:
                            block[size_key] = 225
                    changed = True
            if changed:
